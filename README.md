...
- **Score library** lets you sign in with Clerk to import, save, load, and delete personal MusicXML/MXL files (Supabase). **Public** scores (e.g. River Flows In You) are visible to everyone, including signed-out visitors. Saved scores open in a two-column list with inline sort by date, name, or playback duration (parsed from each score’s MusicXML).
