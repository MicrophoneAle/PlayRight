 ...
 *
 * This fills a real gap, since every other fingering benchmark in this
 * project (chase, repeat-pitch, graced-fixtures snapshot) exercises the
 * RIGHT hand only. There was no LH-specific fingering test before this one.
 *
 * The bug fixed here was that phraseStartCost's LH branch was inverted
 * relative to the already-correct RH branch. signedInterval() normalizes direction so that a
 ...
 * Fable's investigation cited was originally OUT of this fix's reach, because
 * its fifth with finger pair {1,5} triggered OPEN_FRAME_PAIR_BONUS on a
...
describe('LH phrase-start direction bias (phraseStartCost)', () => {
  it('raw cost: an ascending LH phrase start prefers finger 5, not finger 1', () => {
    ...
    // For an ascending start, cost must decrease monotonically from finger 1 to finger 5,
  ...
  it('pure DP: LH ascending stepwise run starts on the pinky (5-4-3-2-1)', async () => {
    // The C4-D4-E4-F4-G4 run has five notes, five fingers, and no crossing/open-frame
  ...
  it('pure DP: river-flows LH broken-chord arpeggio F#3-C#4-F#4 fingers 5-3-1 (open-frame bonus no longer subsidizes the 1-5 crossing)', async () => {
    // This is the named regression target from the LH phrase-start
    // investigation. Before the 2026-07-18 open-frame gating, 1-5-3 (thumb-bottom, pinky crossing
  ...
  it('RH branch is untouched: ascending/descending preference unchanged', () => {
    ...
    // An RH ascending start (low note, ascending next) prefers the low finger (1).
