...
export async function initFingeringModel() {
  if (!session) {
    // This fetches the file directly from your public/ folder
 ...
 */
export async function getMLFingerCosts(phraseNotes: NoteEvent[]): Promise<number[][]> {
  if (!session || phraseNotes.length === 0) return [];
  ...
  const tensor = new ort.Tensor('float32', inputData, [1, seqLength, INPUT_FEATURE_COUNT]);
  const results = await session.run({ note_sequence: tensor });
  ...
  for (let i = 0; i < seqLength; i++) {
    ...
    const expScores = Array.from(noteLogits).map(val => Math.exp(val - maxLogit));
