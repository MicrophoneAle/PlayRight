...
/**
 * Sum of (naturalHeight / naturalWidth) for the page-4 control screenshots.
 * Used so an N-image stack can share one width that still fits the artwork
 * region without scrolling.
 */
const MULTI_IMAGE_STACK_ASPECT_SUM = 86 / 408 + 136 / 417 + 110 / 420;
...
function PageArtwork({ page }: { page: OnboardingPage }) {
  ...
  if (images.length > 1) {
    // Equal-width vertical stack, width capped so the whole stack fits the
    // artwork region (no internal scrollbar).
    return (
      <div
      ...
      >
        <div
          ...
          style={{
            width: `min(100%, calc((100cqh - 0.75rem) / ${MULTI_IMAGE_STACK_ASPECT_SUM}))`,
          }}
        >
          {images.map((image) => (
            <img
    ...
    >
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div
          ...
          className="flex w-full max-w-[544px] shrink-0 flex-col overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl outline-none"
          style={{
